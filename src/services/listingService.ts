...
export interface Listing {
    ...
    // Location fields for GPS-based search
    coordinates?: {
        latitude: number;
        longitude: number;
    };
...
export const listingService = {
        ...
        try {
            ...
            const demoListings: Omit<Listing, 'id' | 'createdAt'>[] = [
                // User Request Specific Items
                {
                    ...
                    coordinates: { latitude: 9.9312, longitude: 76.2673 }, // Kochi coordinates
                    city: 'Kochi',
                    state: 'Kerala',
                    country: 'India',
                    condition: 'New',
                ...
                },
                {
                    ...
                    coordinates: { latitude: 8.5241, longitude: 76.9366 }, // Thiruvananthapuram coordinates
    ...
    // Search listings by location and distance
        ...
        try {
            ...
            const nearbyListings = allListings.filter(listing => {
                if (!listing.coordinates) {
                    // If no coordinates, try to match by city/location string
                    if (listing.location) {
                        const locationLower = listing.location.toLowerCase();
                        // Simple city matching for Kerala cities
                        const keralaCities = ['kochi', 'thiruvananthapuram', 'kozhikode', 'thrissur', 'kollam'];
                        return keralaCities.some(city => locationLower.includes(city));
                    }
                    return false;
                }
            ...
            console.log(`[SERVICE] ✅ Found ${filteredListings.length} listings within ${radiusKm}km`);
    ...
    // Get listings sorted by distance from user location
    getListingsByDistance: async (
        userLocation: { latitude: number; longitude: number },
        limit: number = 20
    ) => {
        try {
            ...
            return listingsWithDistance as Listing[];
        } catch (error) {
            console.error("Error getting listings by distance: ", error);
            throw error;
        }
    },

    // Update listing with location coordinates
